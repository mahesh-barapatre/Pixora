 ...
 */
export async function enhanceQuote(quoteText: string, style?: string): Promise<EnhanceQuoteResult> {
  try {
    ...
    const errorMessage = error instanceof Error ? error.message : 'Failed to enhance quote';
...
export async function generateImageIdeas(
  ...
  try {
    ...
    const errorMessage = error instanceof Error ? error.message : 'Failed to generate image ideas';
 ...
 */
export async function analyzeSentiment(quoteText: string): Promise<SentimentAnalysis> {
  try {
